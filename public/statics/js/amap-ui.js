...
!function(global) {
    ...
    }else if(global.AMap.version==='2.0'){
        throw new Error("JSAPI 2.0 版本下请使用 AMapUI 1.1 版本!");
    }
}(window);
...
AMapUI.define("lib/utils", [], function() {
    ...
    utils = {
        ...
        mergeArray: function(target, source) {
            if (source.length < 50000) target.push.apply(target, source); else for (var i = 0, len = source.length; i < len; i += 1) target.push(source[i]);
        },
